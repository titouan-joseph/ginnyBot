...
def setup(bot):
    print("TGG load")
    bot.add_cog(CogGinny(bot))


class CogGinny(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.sheet = client.open("GinnyBot_Database").sheet1

    @commands.command(aliases=["thomas"])
    async def ginny(self, ctx):
